...
resource "octopusdeploy_kubernetes_cluster_deployment_target" "k8s" {
  ...
  roles               = data.octopusdeploy_environments.all.environments[*].name #After testing change to "[data.octopusdeploy_environments.current.environments[0].id]" because of the license limitation
  ...
  container {
    feed_id = var.octopus_dockerhub_feed_name == "" ? data.octopusdeploy_feeds.docker.feeds[0].id : octopusdeploy_docker_container_registry.GitHub[0].id
