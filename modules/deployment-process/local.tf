...
RELEASENUMBER="$(get_octopusvariable "Octopus.Release.Number")"
DOCKER_IMAGE="$(get_octopusvariable "ecr_url")/$DEPLOYMENT:$RELEASENUMBER"
# Get list of all the containers in the Cronjob including init containers
IFS=" " read -r -a ALL_CONTAINERS <<< "$(kubectl get cronjob __CRONJOB__ -o jsonpath="{.spec.jobTemplate.spec.template.spec.containers[*].name} {.spec.jobTemplate.spec.template.spec.initContainers[*].name}")"
# Container names selected from the value coming from annotation 'octopus.containers.set' in the deployment
OCTOPUS_ANNOTATION=$(kubectl get cronjob __CRONJOB__ -o jsonpath="{.metadata.annotations.octopus\.containers\.set}")
CONTAINERS=()
...
kubectl set image cronjob/__CRONJOB__ $${CONTAINERS[@]}
