...
EOT
  ...
  dynamic "step" {
    for_each = var.enable_slack ? {""} : {} # We iterace once per project if slack is enabled.
    content {
      condition           = "Always"
  ...
  dynamic "step" {
    for_each = var.enable_newrelic ? {""} : {} # We iterace once per project if newrelic is enabled.
    content {
      condition           = "Success"
